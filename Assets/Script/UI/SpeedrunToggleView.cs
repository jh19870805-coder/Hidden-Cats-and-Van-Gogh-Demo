...
/// - 同步 SpeedrunService 的开关状态（读取初始值 / 写入新值）。
/// 使用方式：
...
public class SpeedrunToggleView : MonoBehaviour
{
    ...
    private void Start()
    {
        // 从 SpeedrunService 读取开关初始状态
        if (SpeedrunService.Instance != null)
        {
            _isOn = SpeedrunService.Instance.IsSpeedrunEnabled;
        }
    ...
    {
        // MainWnd FeatureSpec: first-unlock RewardStar on this toggle clears only when the player clicks this control.
        MainMenuUI mainMenu = GetComponentInParent<MainMenuUI>();
        if (mainMenu == null)
        {
            mainMenu = FindFirstObjectByType<MainMenuUI>();
        }

        mainMenu?.DismissSpeedrunUnlockRewardParticlesOnly();
